...
import { resolveWuliArtCookiePath } from './paths.js';
...
export async function read_cookie_file(p: string = resolveWuliArtCookiePath()): Promise<CookieMap | null> {
  try {
...
export async function write_cookie_file(
  cookies: CookieMap,
  p: string = resolveWuliArtCookiePath(),
  source?: string,
): Promise<void> {
